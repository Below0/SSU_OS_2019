...
void timer_handler(struct intr_frame *iframe){
	...
	cur_process->time_slice++; // 프로세스의 총 cpu 점유 tick을 증가
	cur_process->time_used++; // 프로세스의 현재 cpu 사용 tick을 증가

	if(cur_process->time_slice % 30 == 0){ // 30틱마다 우선순위 재계산
			recalculate_priority();
	}

	if(cur_process->time_slice >= TIMER_MAX) // 60틱마다 스케쥴 호출
		do_sched_on_return();
